$(function() {
  ...
  if($('.theme-header').length) {
    var themeHeader = $('.theme-header'),
        ...
        sidebar    = themeBody.find('.theme-sidebar'),
        navLink = sidebar.find('.nav-link');
    ...
    navLink.on('click', function(event) {
      ...
      if($(this).hasClass('nav-first-link')) {
        $(this).addClass('current').parents('.nav-item').siblings().find('.nav-first-link').removeClass('current');
      }
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4) {
          ...
          if($('.ct-sidebar').length) {
            var pages = $('.ct-sidebar').data('index'),
                nav = $('.nav-triggers').eq(pages);
...
//input click시 caancel 버튼 제공
$('.form-search input').on('keyup', function () {
  if ($(this).val().length > 0) {
    $(this).parent().find($('.form-search-cancel')).addClass('active');
  } else {
    $(this).parent().find($('.form-search-cancel')).removeClass('active');
  }
});
//cancel 버튼 클릭시 input value삭제 및 버튼 사라짐
$('.form-search-cancel').on('click', function () {
...
var UXSChart = (function(window) {
  ...
  */
  ...
  function legendCallback(chart) {
    ...
    var className = 'chart-' + chartType + " legend-" + legendPosition;
    var chartClassName = chart.canvas.parentNode.parentNode.className;
    if (chartClassName == className) {
      // var lastChild = findLastChild(chart.canvas.parentNode.parentNode);
      // chart.canvas.parentNode.parentNode.removeChild(lastChild);

      chart.canvas.parentNode.parentNode.removeChild(chart.canvas.parentNode.parentNode.firstChild);

    }

    chart.canvas.parentNode.parentNode.className = chartClassName + " " + className;
    ...
    if(chart.data.datasets.length === 1) {
      ...
      text = getOneDataSetLegend(chart);
  ...
  function getOneDataSetLegend(chart) {

    var text = [];

    var datasets = chart.data.datasets[0];

    for (var i = 0; i < datasets.backgroundColor.length; i++) {
      text.push(
  ...
  return {
    ...
    getLineChartPlugins : function() {
      return lineChartPlugins;
    },
      ...
      for (var i = 0; i < legend.length; i++) {
        ...
        dynamicEventFncMap[i] = function(event) {
          var _this  = this,
              ...
              _parents = _this.closest('.card-body'),
